...
export default function LoginScreen() {
  ...
  const handleAuth = async () => {
    if (!phone || !password) {
      console.log('[Auth] Validation failed: phone or password empty', { phone: !!phone, password: !!password })
      return
    }
    setAuthLock(true)
    setErrorBanner('')
    ...
    try {
      if (isRegister) {
        // Sign up new rider
        console.log('[Auth] Attempting signUp with email:', `${normalizedPhone}@beba.express`)
        const { data: authData, error: signUpError } = await supabase.auth.signUp({
          email: `${normalizedPhone}@beba.express`,
          password: password,
        })
        console.log('[Auth] signUp result:', { authData: JSON.stringify(authData), signUpError: JSON.stringify(signUpError) })
        ...
        if (authData.session) {
          ...
          console.log('[Auth] Profile insert result:', { profileError: JSON.stringify(profileError), userId: authData.user?.id })
          ...
          console.log('[Auth] No session after signUp, email confirmation required')
          setErrorBanner('Check email for confirmation. Auto-confirm in Supabase Dashboard → Auth → Settings.')
          setAuthLock(false)
          return
        }
        ...
        console.log('[Auth] Attempting signInWithPassword for:', `${normalizedPhone}@beba.express`)
        ...
        console.log('[Auth] signInWithPassword succeeded, checking rider role...')
        ...
        console.log('[Auth] User profile check:', { userData: JSON.stringify(userData), profileError: JSON.stringify(profileError) })
  ...
  return (
    <View style={styles.container}>
      <View style={styles.brandContainer}>
        ...
        <Text style={styles.subtext}>{isRegister ? 'Register Rider' : 'Rider Portal'}</Text>
      </View>
      ...
      <TouchableOpacity style={styles.btn} onPress={handleAuth} disabled={authLock}>
        {authLock ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>{isRegister ? 'Register & Start Duty' : 'Start Duty Shift'}</Text>}
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  logoText: { color: '#ffffff', fontSize: 28, fontWeight: '900', letterSpacing: -1 },
  subtext: { color: '#38bdf8', fontSize: 12, fontWeight: '700', textTransform: 'uppercase', marginTop: 4 },
  input: { backgroundColor: '#0f172a', color: '#fff', borderRadius: 8, padding: 16, fontSize: 14, marginBottom: 16, borderWidth: 1, borderColor: '#1e293b', textAlign: 'center' },
  btn: { backgroundColor: '#38bdf8', borderRadius: 8, padding: 16, alignItems: 'center', marginTop: 8 },
  btnText: { color: '#020617', fontSize: 14, fontWeight: '800' },
  errorText: { color: '#ef4444', backgroundColor: '#450a0a', padding: 12, borderRadius: 6, fontSize: 12, fontWeight: '600', marginBottom: 16, textAlign: 'center' },
