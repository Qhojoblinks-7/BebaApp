import React, { useState, useEffect } from 'react';
import { StyleSheet, Text, View, ActivityIndicator, Platform, FlatList, StatusBar } from 'react-native';
import { supabase } from '../../services/supabaseClient';
import { useAuth } from '../../context/AuthContext';
import { Package } from 'lucide-react-native';
import RiderOrderCard from '../../components/rider/RiderOrderCard';
import DeliveryDetailsBottomSheet from '../../components/rider/DeliveryDetailsBottomSheet';

export default function ActiveDeliveryScreen({ navigation }) {
  const { user } = useAuth();
  const [itinerary, setItinerary] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedOrder, setSelectedOrder] = useState(null);

  const fetchActiveItinerary = async () => {
    console.log('[ActiveDelivery] Fetching itinerary for rider:', user?.id)
    const { data, error } = await supabase
      .from('orders')
      .select('*')
      .eq('rider_id', user.id)
      .in('status', ['assigned', 'picked_up', 'in_transit'])
      .order('route_sequence', { ascending: true });

    console.log('[ActiveDelivery] Itinerary result:', { count: data?.length, error: JSON.stringify(error), orders: JSON.stringify(data?.map(o => ({ id: o.id, order_id: o.order_id, status: o.status, route_sequence: o.route_sequence }))) })

    if (!error && data) {
      setItinerary(data);
    }
    setLoading(false);
  };

  useEffect(() => {
    fetchActiveItinerary();
  }, [user?.id]);

  const advanceStatus = async (item) => {
    try {
      if (item.status === 'assigned') {
        const { error } = await supabase
          .from('orders')
          .update({ status: 'picked_up', updated_at: new Date().toISOString() })
          .eq('id', item.id);
        if (error) throw error;
      } else if (item.status === 'picked_up') {
        const { error } = await supabase
          .from('orders')
          .update({ status: 'in_transit', updated_at: new Date().toISOString() })
          .eq('id', item.id);
        if (error) throw error;
      } else if (item.status === 'in_transit') {
        navigation.navigate('DeliveryClosure', { orderId: item.id });
        return;
      }
      fetchActiveItinerary();
    } catch (err) {
      console.warn('[ActiveDelivery] Status advance failed:', err.message);
    }
  };

  if (loading) {
    return (
      <View style={styles.center}>
        <ActivityIndicator size="small" color="#38bdf8" />
      </View>
    );
  }

  if (itinerary.length === 0) {
    return (
      <View style={styles.center}>
        <Package size={32} color="#94a3b8" />
        <Text style={styles.noJobText}>No active dispatches currently ongoing.</Text>
      </View>
    );
  }

  return (
    <View style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor="#115e59" translucent />
      <View style={[styles.headerBackground, styles.safeHeader]}>
        <View style={styles.headerContent}>
          <Text style={styles.titleTag}>Active Run Manifest</Text>
          <Text style={styles.countText}>{itinerary.length} Pending Waypoints Remaining</Text>
        </View>
      </View>

      <FlatList
        data={itinerary}
        keyExtractor={(item) => item.id}
        onRefresh={fetchActiveItinerary}
        refreshing={loading}
        contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 8, paddingBottom: 16 }}
        renderItem={({ item }) => (
          <RiderOrderCard
            order={item}
            onPress={(order) => setSelectedOrder(order)}
          />
        )}
      />

      <DeliveryDetailsBottomSheet
        order={selectedOrder}
        visible={!!selectedOrder}
        onClose={() => setSelectedOrder(null)}
        onAction={(order) => {
          if (!order) return;
          advanceStatus(order);
          setSelectedOrder(null);
        }}
      />
    </View>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#f8fafc' },
  center: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f8fafc' },
  headerBackground: {
    backgroundColor: '#115e59',
    borderBottomLeftRadius: 32,
    borderBottomRightRadius: 32,
    paddingBottom: 24,
    paddingHorizontal: 16,
    ...Platform.select({
      ios: { shadowColor: '#0f172a', shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.15, shadowRadius: 12 },
      android: { elevation: 8 }
    })
  },
  safeHeader: { paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight + 12 : 48 },
  headerContent: { paddingTop: Platform.OS === 'android' ? 16 : 0 },
  titleTag: { fontSize: 11, fontWeight: '900', color: '#ffffff', textTransform: 'uppercase', marginBottom: 4 },
  countText: { fontSize: 16, fontWeight: '800', color: '#ffffff' },
  noJobText: { color: '#64748b', fontSize: 13, fontWeight: '600', marginTop: 12 },
});